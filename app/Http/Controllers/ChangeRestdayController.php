...
class ChangeRestdayController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            $restdaysList = implode(', ', $request->requested_restdays);
            ...
            foreach ($adminUsers as $admin) {
                try {
                    \App\Models\Notification::create([
                        'user_id' => $admin->id,
                        'message' => "{$userName} submitted a change restday request for {$restdaysList}.",
                        'type' => 'change_restday_request_submitted',
                    ]);
                } catch (\Exception $e) {
                    Log::error('Failed to create notification for change restday request', [
                        'admin_id' => $admin->id,
                        'error' => $e->getMessage()
                    ]);
                }
            ...
            Log::error('Error storing change restday request: ' . $e->getMessage(), [
                'user_id' => Auth::id(),
                'request_data' => $request->all(),
                'trace' => $e->getTraceAsString()
            ]);
            return redirect()->back()->with('error', 'Failed to submit change restday request: ' . $e->getMessage())->withInput();
        }
    }
    ...
    public function showEmployeeRestdayHistory(Request $request, \App\Models\User $employee)
    {
        try {
            ...
            Log::error('Error showing employee restday history: ' . $e->getMessage(), [
                'employee_id' => $employee->id ?? null,
                'trace' => $e->getTraceAsString()
            ]);
            return redirect()->back()->with('error', 'Failed to load restday history: ' . $e->getMessage());
        }
    }
}
