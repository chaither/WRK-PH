...
<div class="mx-6 py-6">
    ...
    <form method="GET" action="{{ route('dtr.employee.show', ['employee' => $employee->id]) }}" class="mb-6 flex flex-wrap gap-4 items-end p-3 bg-gray-50 rounded-3xl">
        ...
        <div>
            <label for="start_date" class="block text-xs font-bold text-gray-900 mb-1">Start Date</label>
            <input type="date" name="start_date" id="start_date" value="{{ $startDate ? $startDate->format('Y-m-d') : '' }}" class="border-gray-300 rounded-md px-2 py-1.5 text-sm shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-gray-900">
        </div>
        <div>
            <label for="end_date" class="block text-xs font-bold text-gray-900 mb-1">End Date</label>
            <input type="date" name="end_date" id="end_date" value="{{ $endDate ? $endDate->format('Y-m-d') : '' }}" class="border-gray-300 rounded-md px-2 py-1.5 text-sm shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 text-gray-900">
        </div>
        <div>
            <label for="attendance_date" class="block text-xs font-bold text-gray-900 mb-1">Specific Date:</label>
            <input type="date" name="date" id="attendance_date" value="{{ $selectedDate->format('Y-m-d') }}" class="border-gray-300 rounded-md px-2 py-1.5 text-sm shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-gray-900">
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-2 text-center text-xs font-bold text-gray-700 uppercase tracking-wider">Morning Time In</th>
                        <th class="px-4 py-2 text-center text-xs font-bold text-gray-700 uppercase tracking-wider">Morning Time Out</th>
                        <th class="px-4 py-2 text-center text-xs font-bold text-gray-700 uppercase tracking-wider">Afternoon Time In</th>
                        <th class="px-4 py-2 text-center text-xs font-bold text-gray-700 uppercase tracking-wider">Afternoon Time Out</th>
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    @forelse($records as $record)
                        <tr class="hover:bg-indigo-50 transition duration-100 @if ($loop->even) bg-gray-50 @endif">
                            ...
                            <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-900 text-center">{{ $record->time_in ? \Carbon\Carbon::parse($record->time_in)->format('h:i A') : '-' }}</td>
                            <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-900 text-center">{{ $record->time_out ? \Carbon\Carbon::parse($record->time_out)->format('h:i A') : '-' }}</td>
                            <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-900 text-center">{{ $record->time_in_2 ? \Carbon\Carbon::parse($record->time_in_2)->format('h:i A') : '-' }}</td>
                            <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-900 text-center">{{ $record->time_out_2 ? \Carbon\Carbon::parse($record->time_out_2)->format('h:i A') : '-' }}</td>
                            <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-900 text-center">{{ $record->overtime_time_in ? \Carbon\Carbon::parse($record->overtime_time_in)->format('h:i A') : '-' }}</td>
                            <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-900 text-center">{{ $record->overtime_time_out ? \Carbon\Carbon::parse($record->overtime_time_out)->format('h:i A') : '-' }}</td>
                            <td class="px-4 py-2 whitespace-nowrap text-sm font-bold text-gray-900 text-center">{{ $record->formatted_regular_work_hours }}</td>
