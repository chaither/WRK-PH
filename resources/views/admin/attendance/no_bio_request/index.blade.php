...
<div class="container mx-auto px-4 py-6">
    <h1 class="text-2xl font-semibold text-gray-800 mb-4">Review No Bio Requests</h1>
                    ...
                    @foreach ($noBioRequests as $request)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <form action="{{ route('admin.attendance.no-bio-request.approve', $request->id) }}" method="POST" class="inline-block">
                                    @csrf
                                    <button type="submit" class="text-green-600 hover:text-green-900 mr-3">Approve</button>
                                </form>
                                <form action="{{ route('admin.attendance.no-bio-request.reject', $request->id) }}" method="POST" class="inline-block">
                                    @csrf
